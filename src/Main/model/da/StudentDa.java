...
public class StudentDa implements AutoCloseable, CRUD<Student> {
    ...
    public Student save(Student student) throws Exception {
        ...
        preparedStatement = connection.prepareStatement(
                "INSERT INTO STUDENT(ID, NAME, FAMILY, GENDER, BIRTHDATE, CITY, PHONENUMBER, EMAIL, ADDRESS,COURSE) VALUES (?,?,?,?,?,?,?,?,?,?)"
        );
    ...
    public Student edit(Student student) throws Exception {
        preparedStatement = connection.prepareStatement(
                "UPDATE STUDENT SET NAME=?,FAMILY=?,GENDER=?,BIRTHDATE=?,CITY=?,PHONENUMBER=?,EMAIL=?,ADDRESS=?,COURSE=? WHERE ID=?"
        );
